...
public class scenario3Test {
    ...
    @Test
    public void createAPetWithBody() throws Exception{
        FileReader csvFileReader = new FileReader("scenario3Test.csv");
        Iterable<CSVRecord> csvRecords = CSVFormat.DEFAULT.withDelimiter('^').withQuote('"').withFirstRecordAsHeader().parse(csvFileReader);
        Iterator<CSVRecord> csvRecordItr = csvRecords.iterator(); 
        csvRecordItr.next(); //"Skipped header."
        while (csvRecordItr.hasNext()) {
            CSVRecord csvRecord = csvRecordItr.next();
            if(csvRecord.get("METHOD").equalsIgnoreCase("POST")) {

                String url = csvRecord.get("URL");
                JSONObject req_headers = new JSONObject(csvRecord.get("REQ_HEADERS"));
                JSONObject req_body = new JSONObject(csvRecord.get("REQ_BODY"));
                int response_code = Integer.parseInt(csvRecord.get("RESPONSE_CODE"));
                JSONObject res_body = new JSONObject(csvRecord.get("RESPONSE_BODY"));

                // Send request and receive Response.
                Response response = given().headers(req_headers.toMap()).body(req_body.toString()).when().post(url);
                // Get Response body (to do assertion for validating the response body later).
                JsonPath responseJson = response.jsonPath(); 
                // Assert for the status code.
                Assert.assertEquals("API returned unexpected status.", response_code, response.getStatusCode());

                // Place the response body in JSONObject , to access it's elements and assert.
                JSONObject resExpected = new JSONObject(res_body.toString());
                // Assert the actual response body with expected response body.
                Assert.assertEquals(resExpected.getString("name"), responseJson.getString("name") );
                Assert.assertEquals(resExpected.getString("status"), responseJson.getString("status") );
                Assert.assertEquals(resExpected.getString("id"), responseJson.getString("id") );

                // Assert the response time to be within expected limit.
                Assert.assertTrue("API response time is more than 50ms.", response.getTime() <= 50);
            }
        }
    }
}
